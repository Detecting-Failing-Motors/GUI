...
#Import Classifiers
#More info at https://scikit-learn.org/stable/supervised_learning.html#supervised-learning
#Commented Out Classifiers refer to models that gave an error
...
def GenerateIMSDictionary(FileOfInterest,TrainingDataFile,HomeDirectory):
    ...
    UserInput = UserInputs2WorkingForm(n,N,Bd,Pd,phi,SampleFrequency,FileName,data,HomeDirectory,directory,TrainingDataFile)
#UserInputs2WorkingForm(self.n,self.N,self.Bd,self.Pd,self.phi,self.SampleFrequency,Data,self.HomeDirectory,self.directory,self.TrainingDataFile,self.inputSavingalias,self.inputName,self.inputModelnum)
    return UserInput
...
def UserInputs2WorkingForm(n,N,Bd,Pd,phi,SampleFrequency,RawData,HomeDirectory,directory,TrainingDataFile,save,name_id,modelnum):
    """
...
#    figs.append(getPlot(x1['Frequency'],x1['Freq. Amp.'],'Frequency [Hz]',"Normalized Amplitude","FFT"))
#    figs.append(getPlot(x2['Frequency'],x2['PSD'],'Frequency [Hz]','Normalized PSD [V**2 / Hz]',"PSD"))
#    figs.append(getPlot(x3['X Values'],x3['Autocorr Values'],'time delay [s]',"Normalized Autocorrelation amplitude","Autocorrelation"))
...
#    return figs
    ...
    plot5info = []
    plot6info = []
    #getPlot(t,UserInput['Signal Data of Interest'],"time (s)","Amplitude","Raw Data",GUI6.fig0)
    plotinfo.append(t)
    plotinfo.append(UserInput['Signal Data of Interest'])
    plotinfo.append("time (s)")
    plotinfo.append("Amplitude")
    plotinfo.append("Raw Data")
                                    
    #getPlot(t,UserInput1['Signal Data of Interest'],"time (s)","Amplitude","Raw Data w/ Removed DC Offset",stringfigs[1])
    plot2info.append(t)
    plot2info.append(UserInput1['Signal Data of Interest'])
    plot2info.append("time (s)")
    plot2info.append("Amplitude")
    plot2info.append("Raw Data w/ Removed DC Offset")

    
    #getPlot(x1['Frequency'],x1['Freq. Amp.'],'Frequency [Hz]',"time (s)","FFT",stringfigs[2])
    plot3info.append(x1['Frequency'])
    plot3info.append(x1['Freq. Amp.'])
    plot3info.append('Frequency [Hz]')
    plot3info.append("time (s)")
    plot3info.append("FFT")
    
    #getPlot(x2['Frequency'],x2['PSD'],'Frequency [Hz]','PSD [V**2 / Hz]',"PSD",stringfigs[3])
    plot4info.append(x2['Frequency'])
    plot4info.append(x2['PSD'])
    plot4info.append('Frequency [Hz]')
    plot4info.append('PSD [V**2 / Hz]')
    plot4info.append("PSD")
    
    
    #getPlot(x3['X Values'],x3['Autocorr Values'],'time delay [s]',"Autocorrelation amplitude","Autocorrelation",stringfigs[4])
    plot5info.append(x3['X Values'])
    plot5info.append(x3['Autocorr Values'])
    plot5info.append('time delay [s]')
    plot5info.append("Autocorrelation amplitude")
    plot5info.append("Autocorrelation")
    
    #getPlot(t,UserInput2['Signal Data of Interest'],"time (s)","Amplitude","Normalized Raw Data")
    plot6info.append(t)
    plot6info.append(UserInput2['Signal Data of Interest'])
    plot6info.append("time (s)")
    plot6info.append("Amplitude")
    plot6info.append("Normalized Raw Data")

    
    return plotinfo,plot2info,plot3info,plot4info,plot5info,plot6info
...
def GetFinalEightModelsForComparison(MinSampleSplit=2):
    """
    Returns a dictionary of the 8 best possible ML classifiers with ability to change the 
    min_samples_split setting
    
    GetFinalEightModelsForComparison(
        MinSampleSplit - integer to change min_samples_split (default = 2)
        )
...
def GetOnlyTwoModelsForComparison():
    """
    Returns a dictionary of just TWO possible ML classifiers with "default" settings
    
    GetOnlyTwoModelsForComparison()
...
def CreateSaveDictionary(inputName,inputApplication,inputModelnum,inputSavingalias,inputFile,mlData, horsepower, voltage, phase, shaftnum,shaftSpeed,numberofElements, diameterofElements, pitchDiameter, contactAngle, samFreq):

    x = {
         "inputName": inputName,
         "inputApplication": inputApplication,
         "inputModelnum": inputModelnum,
         "inputSavingalias": inputSavingalias,
         "inputFile": inputFile,
         "mlData": mlData,
         ...
         "diameterofElements": diameterofElements,
